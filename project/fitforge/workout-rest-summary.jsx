...
// ---------- timer de repos ----------
function RestTimer({ total, design, exerciseName, onDone, onSkip }) {
  ...
  const adjust = (d) => { setLeft((l) => Math.max(1, l + d)); setMax((m) => Math.max(m, left + d)); };
  const p = left / max;
...
function IntensityChart({ samples }) {
  const W = 320, H = 90;
  const px = (i) => 8 + (i / (samples.length - 1)) * (W - 16);
  const py = (v) => H - 12 - (v / 4) * (H - 28); // intensité 0-4
...
// ---------- récap post-séance ----------
function SummaryScreen({ session, onShare, onHome }) {
...
Object.assign(window, { RestTimer, SummaryScreen, IntensityChart, ShareCard });
