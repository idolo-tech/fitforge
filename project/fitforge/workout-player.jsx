  ...
  return (
    <div style={{ position: 'relative', height: 56 }}>
      <div
        ...
        onTouchEnd={end}
        onMouseDown={(e) => { begin(e.clientY); const mv = (ev) => move(ev.clientY); const up = () => { end(); window.removeEventListener('mousemove', mv); window.removeEventListener('mouseup', up); }; window.addEventListener('mousemove', mv); window.addEventListener('mouseup', up); }}
...
// ---------- player principal ----------
function WorkoutPlayer({ gesture, timerDesign, onFinish, onQuit }) {
  const day = window.FF.todayDay;
  const exercises = day.exercises;

  const [exIdx, setExIdx] = React.useState(0);
  const [setNum, setSetNum] = React.useState(1);
  const [logs, setLogs] = React.useState([]); // {exIdx, set, weight, reps, rir}
  ...
  const range = window.FF.repsRange(ex.reps);
  ...
  React.useEffect(() => {
    ...
    const r2 = window.FF.repsRange(e2.reps);
    setReps(r2 ? r2[0] : 10);
    setRir(2);
  }, [exIdx]);
  ...
  const rirColor = (r) => (r <= 0 ? '#FF9F43' : r <= 2 ? '#39FF14' : '#4A4A4A');
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, zIndex: 30, background: 'var(--bg-0)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}
      ...
      <style>{`@keyframes ff-slide-ex { from { opacity: 0; transform: translateX(46px); } to { opacity: 1; transform: translateX(0); } }`}</style>
    </div>
  );
}

Object.assign(window, { WorkoutPlayer, ValidateControl, fmtClock });
