...
function LiftChart({ series, active }) {
  const W = 320, H = 130;
  ...
  const maxWeek = 4;
  const px = (w) => 10 + ((w - 1) / (maxWeek - 1)) * (W - 20);
...
function JournalScreen() {
  const { weeklyVolume, liftSeries, weeks, history } = window.FF;
  const [period, setPeriod] = React.useState('Mois');
  const [active, setActive] = React.useState(new Set(['dev-incline', 'squat-guide']));
  ...
  const toggleLift = (id) => setActive((a) => { const s = new Set(a); s.has(id) ? s.delete(id) : s.add(id); return s; });
              ...
              <div key={day.iso} className="ff-card">
                <button className="pressable" onClick={() => setExpanded(isExp ? null : day.iso)} style={{ width: '100%', padding: '14px 16px', display: 'flex', alignItems: 'center', gap: 14, textAlign: 'left' }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div className="ff-mono" style={{ fontSize: 11, color: 'var(--txt-2)', marginTop: 3, textTransform: 'capitalize' }}>{window.FF.fmtLong(day.date)} · {s.duration} min</div>
                  </div>
                  {s.volume > 0 && <span className="ff-mono" style={{ fontSize: 14, fontWeight: 700, color: 'var(--accent)' }}>{(s.volume / 1000).toFixed(1)} t</span>}
...
Object.assign(window, { JournalScreen, VolumeBarChart, LiftChart });
