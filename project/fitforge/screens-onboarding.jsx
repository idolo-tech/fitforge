...
function ForgeLogo({ size = 96, animate = true }) {
  const common = {
  ...
  return (
    <svg width={size} height={size} viewBox="0 0 96 96" style={{ filter: 'drop-shadow(0 0 calc(14px * var(--glow)) rgba(0,240,255,0.5))' }}>
      <polygon points="48,6 88,28 88,68 48,90 8,68 8,28" {...common}
        style={animate ? { animation: 'ff-draw calc(1.1s * var(--speed)) ease-out forwards' } : null} />
      <path d="M36 66 V32 H62 M36 48 H56" {...common} strokeDasharray="120" strokeDashoffset={animate ? 120 : 0}
        style={animate ? { animation: 'ff-draw calc(0.8s * var(--speed)) calc(0.7s * var(--speed)) ease-out forwards' } : null} />
    </svg>
  );
}
...
function WheelColumn({ values, value, onChange, width = 80 }) {
...
function OnboardingScreen({ onDone }) {
  ...
  const [month, setMonth] = React.useState('Mars');
  const [year, setYear] = React.useState(1998);
  const [weight, setWeight] = React.useState(70);
  const [height, setHeight] = React.useState(178);
  const [goal, setGoal] = React.useState('muscle');
  const [equip, setEquip] = React.useState(['gym']);
          ...
          <>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'center', gap: 8, background: 'var(--bg-1)', border: '1px solid var(--line)', borderRadius: 16, padding: '8px 12px' }}>
                ...
                <WheelColumn values={window.FF.MONTHS_FR.map((m) => m[0].toUpperCase() + m.slice(1, 4))} value={month} onChange={setMonth} width={84} />
...
Object.assign(window, { SplashScreen, OnboardingScreen, ForgeLoading, ForgeLogo, WheelColumn, MagneticSlider });
