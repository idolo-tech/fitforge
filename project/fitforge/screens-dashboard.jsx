...
function DashHeader({ name }) {
  const { TODAY, streak, fmtLong } = window.FF;
  return (
    <header style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '22px 20px 6px' }}>
...
function HeroSeance({ layout, onStart }) {
  const day = window.FF.todayDay;
  if (!day) return null;
  ...
  if (layout === 'Compact') {
    return (
      <section className="ff-card anim-fade-up" style={{ margin: '14px 20px 0', padding: 20, display: 'flex', flexDirection: 'column', gap: 14 }} aria-label="Séance du jour">
  ...
  if (layout === 'Split') {
    return (
      <section className="ff-card anim-fade-up" style={{ margin: '14px 20px 0', overflow: 'hidden', display: 'grid', gridTemplateColumns: '104px 1fr' }} aria-label="Séance du jour">
  ...
  // Immersif (défaut)
  return (
    <section className="anim-fade-up" style={{ margin: '14px 20px 0', borderRadius: 'var(--r-lg)', overflow: 'hidden', position: 'relative', border: '1px solid var(--line)' }} aria-label="Séance du jour">
      <ExercisePlaceholder label={`photo — ${day.short.toLowerCase()}`} height={290} />
...
function WeekCalendar() {
  const { TODAY, weeks, history, fmtISO, DAYS_FR_SHORT } = window.FF;
  // semaine courante : lundi 6 juillet
  const monday = weeks[3].monday;
  const days = Array.from({ length: 7 }, (_, i) => window.FF.addDays(monday, i));
  const trainISO = new Set(weeks[3].days.map((d) => d.iso));
  return (
    <div style={{ display: 'flex', gap: 8, padding: '18px 20px 0', overflowX: 'auto', scrollSnapType: 'x mandatory' }} aria-label="Calendrier de la semaine">
      {days.map((d) => {
        const iso = fmtISO(d);
...
function WeekBars() {
  const { weeks, history, TODAY, fmtISO } = window.FF;
  const week = weeks[3];
  const done = week.days.filter((d) => history[d.iso] && history[d.iso].status === 'completed').length;
  return (
    <section style={{ margin: '22px 20px 0' }} aria-label="Cette semaine">
...
/* ---------- progression : scroll horizontal ---------- */
function ProgressCards() {
  const { weeklyVolume, lastPR, bodyWeight } = window.FF;
  const weekVol = weeklyVolume[3].volume || weeklyVolume[2].volume;
  const cardStyle = { minWidth: 168, padding: 16, display: 'flex', flexDirection: 'column', gap: 10, flexShrink: 0 };
  return (
    <section style={{ margin: '22px 0 0' }} aria-label="Progression">
      <div className="ff-label" style={{ margin: '0 20px 12px' }}>Progression</div>
      <div style={{ display: 'flex', gap: 12, overflowX: 'auto', padding: '0 20px 6px' }}>
        ...
        <div className="ff-card" style={{ ...cardStyle, minWidth: 230 }}>
          <span className="ff-label" style={{ fontSize: 10 }}>Consistance · 12 semaines</span>
          <ConsistencyHeatmap cell={10} gap={3} />
          <span style={{ fontSize: 11, color: 'var(--txt-1)' }} className="ff-mono">{window.FF.completedCount} séances · 1 manquée</span>
        </div>
      </div>
    </section>
  );
...
function DashboardScreen({ name, heroLayout, onStartWorkout }) {
  return (
...
Object.assign(window, { DashboardScreen, HeroSeance, WeekCalendar, WeekBars, ProgressCards, DashHeader });
